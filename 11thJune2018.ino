...
int attendance()
{
 ...
 for(int i=0;i<=studCounter;i++)//studCounter starts from 0
 {
  ...
  timeNow();
  dailyAttendance(stud.regNo[i]);//keep the attendance record in the daily attendance file
...
//if at midnight then create a new file addon to the list.csv
...
void dailyAttendance(long regNo)
{
...
//ALL I WANT FROM THE BELOW FUNCTION 1.user's line number in the list.csv,2.fingerprint pressed times,3.time the attendance was due
void fileDownload()
{
...
/*
   ...
   File listFile=("list.csv",FILE_WRITE);
        if(listFile)//if the listFile exists
        while(listFile.available())//while the file is available for operation
        {
          
        }
        */
      File todayFile=SD.open(getNameExt,FILE_READ);//open the file in reading mode
...
//addon to be added on the file.csv at the mid night for the newday's attendance
void fileAddon()
{
  ...
  //if at the midnight.......add to file.csv
  
    //------ADDING THE TIME AND COMMENT SPACE IN THE file.csv------------------------
   ...
   File fileEdit=SD.open("list.csv");
    ...
    if(fileEdit)
    {
        ...
        if(x==',')
          {
           ...
           if(commaCounter==endOLine)
           {
             if(line ==1)
             {
              ...
              //fetch the time in 6/11/2018 from the time
              DateTime now = rtc1.now();
              
              char getNowDate[10];
              
              sprintf(getNowDate,"%d/%d/%d",now.day(),now.month(),now.year());
              fileEdit.print(",");
              fileEdit.print(getNowDate);
              fileEdit.print(",,COMMMENT,");
              
              Serial.print(",");
              Serial.print(getNowDate);
             ...
             if(line==2)
             {
              ...
              fileEdit.print(",TIME IN,TIME OUT,,");
              Serial.print(",TIME IN,TIME OUT,,");
      ...
      fileEdit.close();
        ...
        //record the value into EEPROM
/*     
 fileEdit=SD.open("list.csv",FILE_READ);
 if(fileEdit)
 {
  while(fileEdit.available())
  {
...
void downloadableFile(int line,int pressedTimes,String Time)
{
  ...
  //open the list.csv
  File fileEdit = SD.open("list.csv",FILE_WRITE);
  {
    while(fileEdit.available())
    {
      char x=fileEdit.read();
